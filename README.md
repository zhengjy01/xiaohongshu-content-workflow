...
- **三道人工审核闸门，agent 不得自动跨越**。整条流水线被三道闸门切开：①**每日选题**（雷达写完即停，由用户挑一条）②**生图前审提示词与张数**（模板 / 提示词全文 / 计划张数及用途 / 尺寸 1080×1440；未放行前禁止打开 ChatGPT 网页版）③**发布/打回**（由用户决定）。闸门 2、3 都允许打回，**打回必须带原因**；打回后 agent 先读 `review_note` 修订，再把状态放回被卡住的闸门。详见 `docs/小红书三道审核闸门-SOP.md`。
...
- **内容命名与归档统一**。日期开头 + 实词关键词的命名规则、系列文件夹的拆分阈值、`status` 状态机（选题 `待选题审核 → 已选题 / 已搁置`；笔记 `待生图审核 → 待发布审核 → 已发布 / 已打回`，打回修订期置 `打磨中`）原地推进，不搬文件。
...
├── docs/
│   ├── 小红书三道审核闸门-SOP.md      # 三道人工闸门的操作口径（状态取值 / 打回闭环 / 与手机审核台协作）
...
| Skill | 作用 | 关键点 |
| --- | --- | --- |
| `xiaohongshu-content-pipeline` | 端到端流水线 | 确认痛点 → 研究 → 成稿 → 配图 → 存附件 → 呈现；三道审核闸门总纲 |
| `xiaohongshu-city-research` | 研究底座 | 固定口径、来源优先级、证据标签、生活成本模型、可比性检查、选题评分、成稿/图卡模板、选题雷达；闸门 1「写完即停」 |
| `xiaohongshu-image-card` | 配图生成 | style-library 出提示词 → **闸门 2 审核通过** → ChatGPT 网页版真生图 → 存附件库 → 笔记嵌入 |
| `content-output-naming` | 命名归档 | `YYYY-MM-DD-关键词1-关键词2`、系列文件夹、`status` 状态机（含三道闸门取值） |
